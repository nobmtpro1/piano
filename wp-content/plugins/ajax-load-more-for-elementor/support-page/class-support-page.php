 ...
 *******************************/
add_action('wp_head', 'pd_alm_support_form_ajax_header');

if( !function_exists('pd_alm_support_form_ajax_header') )
{
	function pd_alm_support_form_ajax_header() 
	{
	...
	} //End of pd_alm_support_form_ajax_header
 ...
 *******************************/
add_action( 'wp_ajax_process_pd_alm_promo_form', 'process_pd_alm_promo_form' );

if( !function_exists('process_pd_alm_promo_form') )
{
	function process_pd_alm_promo_form()
	{
 ...
 *******************************/
if( !class_exists('WBELALMSupportPage') ){


	class WBELALMSupportPage{
		...
		function __construct( $plugin_slug = null )
		{
			...
			//add_action( 'wp_ajax_process_pd_alm_promo_form', array(&$this,'process_pd_alm_promo_form') );
			
		} //End of Constructor
		
		function include_promo_page_scripts( $hook )
		{                                 
		   if( $hook == 'ajax-load-more-for-elementor_page_wb-page-pd_alm-support' ){	   	
		...
		function show_promo_page_callback_func()
		{
			add_submenu_page(
				...
				'manage_options',
				$this->page_slug,
				array(&$this, 'pd_almpromo_support_page_callback_func' )
			);
		 ...
		 *******************************/
		function pd_almpromo_support_page_callback_func()
		{
			
			?>
				
				<div class="pd_alm-support-page-wrapper">
			...
			<?php
		} //End of pd_almpromo_support_page_callback_function
		
		
	
	} //End of the class WBELALMSupportPage
...
//Supply Unique Promo Page Slug as the constructor parameter of the class WBELALMSupportPage. ex: sld-page-2124a to the constructor

//Please create an unique instance for your use, example: $instance_sldf2

$instance = new WBELALMSupportPage('pd_alm-support');

if( is_admin() )
{
	$instance->plugin_menu_slug = "pd-ajax-load-more"; //Edit Value
	$instance->plugin_name = "Ajax Load More for Elementor"; //Edit Value
